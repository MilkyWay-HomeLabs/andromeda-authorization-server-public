...
public interface AccessTokenBuilder {
    ...
    AccessTokenBuilder userId(long userId);

    AccessTokenBuilder token(String token);

    AccessTokenBuilder expirationDate(Timestamp expirationDate);

    AccessTokenBuilder version(int version);

    AccessTokenBuilder jti(String jti);

    AccessTokenBuilder revoked(boolean revoked);
