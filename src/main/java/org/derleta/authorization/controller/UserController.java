...
public class UserController {
    ...
    public ResponseEntity<UserResponse> add(@RequestBody UserRequest userRequest) {
        User created = service.save(UserApiMapper.toUserFromRequest(userRequest));
        UserResponse response = userModelAssembler.toModel(created);
     ...
     * @param userRequest the new details of the user to update
    ...
    public ResponseEntity<UserResponse> update(@PathVariable Long id, @RequestBody UserRequest userRequest) {
        User patch = UserApiMapper.toUserPatch(userRequest);
        User updated = service.update(id, patch);
        if (updated == null) {
            return ResponseEntity.notFound().build();
        }
        UserResponse response = userModelAssembler.toModel(updated);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
