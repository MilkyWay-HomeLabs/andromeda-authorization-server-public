...
public class RefreshTokenController {
     ...
     * @param sortBy         the field by which to sort the results, a default is "userId"
     * @param sortOrder      the order of sorting, either "asc" (ascending) or "desc" (descending), a default is "asc"
    ...
    public ResponseEntity<RefreshTokenResponse> add(@PathVariable Long userId) {
        RefreshToken refreshToken = service.createForUser(userId);
        if (refreshToken == null) {
            return ResponseEntity.notFound().build();
        }
        RefreshTokenResponse response = tokenModelAssembler.toModel(refreshToken);
