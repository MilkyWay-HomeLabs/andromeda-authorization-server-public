...
class AccessTokenServiceTest {
    ...
    private AccessTokenRepository mockRepository;
    private AccessTokenService tokenService;

    @BeforeEach
    void setUp() {
        mockUserRepository = mock(UserRepository.class);
        mockRepository = mock(AccessTokenRepository.class);
    ...
    void getPage_withValidParameters_shouldReturnCorrectResult() {
        ...
        List<TokenEntity> tokenEntities = List.of(
                new AccessTokenEntity(1L, new UserEntity(), "token1", null),
                new AccessTokenEntity(2L, new UserEntity(), "token2", null)
        );
        ...
        // Act
        Page<AccessToken> result = tokenService.getPage(page, size, sortBy, sortOrder, usernameFilter, emailFilter);
    ...
    void getValid_withValidParameters_shouldReturnCorrectResult() {
        ...
        // Act
        Page<AccessToken> result = tokenService.getValid(page, size, sortBy, sortOrder);
    ...
    @Test
    void get_withValidTokenId_shouldReturnAccessToken() {
        // Arrange
        long tokenId = 123L;
        AccessTokenEntity tokenEntity = new AccessTokenEntity(tokenId, new UserEntity(), "validToken", null);
        when(mockRepository.findById(tokenId)).thenReturn(Optional.of(tokenEntity));

        // Act
        AccessToken result = tokenService.get(tokenId);
    ...
    void save_withValidData_shouldSaveTokenAndReturnCorrectResult() {
        ...
        long tokenId = 123L;
        AccessTokenEntity expectedEntity = new AccessTokenEntity(tokenId, userEntity, token, null);

        when(mockUserRepository.isValidId(userId)).thenReturn(true);
        when(mockRepository.getNextId()).thenReturn(tokenId);
        when(mockRepository.findById(tokenId)).thenReturn(Optional.of(expectedEntity));

        // Act
        AccessToken result = tokenService.save(userId, token);
    ...
    void save_withNullToken_shouldReturnNull() {
        ...
        // Act
        AccessToken result = tokenService.save(userId, null);
    ...
    void delete_withValidTokenIdAndUserId_shouldDeleteToken() {
        ...
        long tokenId = 1L;
        long userId = 1L;
        TokenEntity tokenEntity = new AccessTokenEntity();
