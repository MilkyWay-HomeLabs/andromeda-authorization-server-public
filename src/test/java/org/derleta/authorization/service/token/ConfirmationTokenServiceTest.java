...
class ConfirmationTokenServiceTest {
    ...
    private ConfirmationTokenRepository mockRepository;
    private ConfirmationTokenService tokenService;

    @BeforeEach
    void setUp() {
        mockUserRepository = mock(UserRepository.class);
        mockRepository = mock(ConfirmationTokenRepository.class);
    ...
    void getPage_withValidParameters_shouldReturnCorrectResult() {
        ...
        List<TokenEntity> tokenEntities = List.of(
                new ConfirmationTokenEntity(1L, new UserEntity(), "token1", null),
                new ConfirmationTokenEntity(2L, new UserEntity(), "token2", null)
        );
        ...
        // Act
        Page<ConfirmationToken> result = tokenService.getPage(page, size, sortBy, sortOrder, usernameFilter, emailFilter);
    ...
    void getValid_withValidParameters_shouldReturnCorrectResult() {
        ...
        // Act
        Page<ConfirmationToken> result = tokenService.getValid(page, size, sortBy, sortOrder);
    ...
    @Test
    void get_withValidTokenId_shouldReturnConfirmationToken() {
        // Arrange
        long tokenId = 123L;
        ConfirmationTokenEntity tokenEntity = new ConfirmationTokenEntity(tokenId, new UserEntity(), "validToken", null);
        when(mockRepository.findById(tokenId)).thenReturn(Optional.of(tokenEntity));

        // Act
        ConfirmationToken result = tokenService.get(tokenId);
    ...
    void save_withValidData_shouldSaveTokenAndReturnCorrectResult() {
        ...
        when(mockRepository.findById(tokenId)).thenReturn(Optional.of(new ConfirmationTokenEntity(tokenId, userEntity, token, null)));

        // Act
        ConfirmationToken result = tokenService.save(userId, token);
    ...
    @Test
    void save_withoutNextId_shouldReturnNull() {
        // Arrange
        long userId = 1L;
        String token = "validTokenString";
    ...
    void save_withNullToken_shouldReturnNull() {
        ...
        // Act
        ConfirmationToken result = tokenService.save(userId, null);
    ...
    @Test
    void delete_withValidTokenIdAndUserId_shouldDeleteToken() {
        // Arrange
        long tokenId = 1L;
        long userId = 1L;
        TokenEntity tokenEntity = new ConfirmationTokenEntity();
