...
class ConfirmationTokenRepositoryTest {
    ...
    @Test
    void setExpiredTokenNow_withExistingTokenId_shouldSetExpirationDateCloseToNow() {
        long id = 8;

        TokenEntity beforeUpdate = repository.findById(id).orElse(null);
        assertNotNull(beforeUpdate);
        assertNotNull(beforeUpdate.getExpirationDate());

        var previousExpiration = beforeUpdate.getExpirationDate().toInstant();

        int result = repository.setExpired(id);

        TokenEntity afterUpdate = repository.findById(id).orElse(null);
        assertEquals(1, result);
        assertNotNull(afterUpdate);
        assertNotNull(afterUpdate.getExpirationDate());

        var updatedExpiration = afterUpdate.getExpirationDate().toInstant();

        assertNotEquals(previousExpiration, updatedExpiration);
        assertTrue(updatedExpiration.isBefore(java.time.Instant.now().plusSeconds(5)));
    }
