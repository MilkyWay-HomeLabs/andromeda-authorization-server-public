...
As of version 3.5.0, certificates are managed automatically via **Traefik**. Manual certificate management and truststore configuration are no longer required for standard deployments.

### 📦 Archived: Self-Signed Certificate Setup (v3.1.1 and older)
> [!NOTE]
> This section is kept for archival purposes and applies only to versions 3.1.1 and older, or environments where secrets are not exposed.
