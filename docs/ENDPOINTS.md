...
8. [REST Examples](#rest-examples)
9. [IntelliJ HTTP Client Files](#intellij-http-client-files)
...
### Reset Password
- **URL:** `/public/account/reset-password/{email}`
- **Method:** `PATCH`
...
- **Method:** `POST`
- **Authorization:** `ROLE_USER`, `ROLE_TESTER`, `ROLE_MODERATOR`, `ROLE_ADMIN`
...
- `GET /`: Paginated list (Admin/Tester)
- `GET /valid`: Paginated valid tokens (Admin/Tester)
- `GET /{id}`: Get by ID (Admin/Tester)
- `POST /{userId}`: Add for user (Admin/Tester/Moderator)
